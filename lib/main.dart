...
class _SelectPageState extends State<SelectPage> {
  ...
  //http://backendforecs-1001704613.ap-northeast-2.elb.amazonaws.com/api/create/first_post/user_tbl?Model_1_rst=1&Model_3_rst=1
  ...
  //
  //
  //   Uri url = Uri.parse('http://backendforecs-1001704613.ap-northeast-2.elb.amazonaws.com/api/create/first_post/user_tbl?Model_1_rst=1&Model_3_rst=1');
  ...
  Future PostData() async {
    ...
    Uri url = Uri.parse('http://3.34.239.212/api/create/first_post/user_tbl?Model_1_rst=1&Model_3_rst=1');
    http.Response response = await http.post(url,
        headers: <String, String>{
          'Content-Type': 'application/json; sharset=UTF-8',
        }, //this. header is essential to send json data
  ...
  getData()async{
    var url =await http.get(Uri.parse('http://3.34.239.212/api/create/first_post/user_tbl?Model_1_rst=1&Model_3_rst=1'));
        ...
        appBar: AppBar(backgroundColor: Colors.black,actions: [
          TextButton(onPressed: ()async{
            ...
            if((selectedMBTI != null)&(selectedDate != null)&(selectedMBTI != MBTI[0])&(widget.UserImagePath != null))
            {
              ...
              await getData();
              Navigator.push(context, MaterialPageRoute(builder: (c){
                return ResultPage(resultData:resultData);
              }));
            } else
              showDialog(context: context, builder: (context){
...
class ResultPage extends StatelessWidget {
  ...
  ResultPage({Key? key,this.resultData}) : super(key: key);
          ...
          body: ListView(
            children: [
              Center(child: Column(children: [
                ...
                Container(alignment: Alignment.center,width: 300,height: 100,color: Colors.blue,child: Text('ResultClass Text',style: TextStyle(fontSize: 30),)),
                SizedBox(height: 10,),
                Container(width: 400,height: 150,color: Colors.blue,child: Text('설명글',style: TextStyle(fontSize: 30),)),
                SizedBox(height: 10,),
                Container(width: double.infinity,height: 300,color: Colors.blue,child: Image.network(resultData[0]['image'],fit: BoxFit.fill,)),
                SizedBox(height: 10,),

                Row(mainAxisAlignment: MainAxisAlignment.spaceAround,children: [

                  Container(height: 200,width: 200,color: Colors.green,child: Text('class face'),),
                  Container(height: 200,width: 200,color: Colors.green,child: Text('class face'),),
                ]),
                SizedBox(height: 10,),
                Row(mainAxisAlignment: MainAxisAlignment.spaceAround,children: [
